...
//        description.setText("需要展示的内容");
        ...
        leftYAxis.setEnabled(true); //左侧Y轴不显示
        /***折线图例 标签 设置***/
    ...
    public void showLineChart(final List<CompositeIndexBean> dataList, String name, int color) {
        ...
        float max = 0;
        if(dataList.size() > 0){
            min = (float)dataList.get(0).getValue();
            max = (float)dataList.get(0).getValue();
        }
        ...
        for (int i = 0; i < dataList.size(); i++) {
            ...
            if(min > dataList.get(i).getValue()){
                min = (float)dataList.get(i).getValue();
            }
            if(max < dataList.get(i).getValue()){
                max = (float)dataList.get(i).getValue();
            }
        }
        ...
        xAxis.setValueFormatter(new IAxisValueFormatter() {
            @Override
            public String getFormattedValue(float value, AxisBase axis) {
                String tradeDate = dataList.get((int) value % dataList.size()).getDataDT();

                return TextUtils.formatDateToMD(tradeDate,"yyyy-MM-dd HH:mm:ss","HH:mm");
            }
        });
        ...
        leftYAxis.setAxisMinimum(min-1); // start at zero
        leftYAxis.setAxisMaximum(max+1); // the axis maximum is 100
     ...
     * @param color    曲线颜色
     */
    public void showLineChartY(final List<CompositeIndexBean> dataList, String name, int color,float min,float max) {
        List<Entry> entries = new ArrayList<>();
        ...
        xAxis.setValueFormatter(new IAxisValueFormatter() {
            ...
            public String getFormattedValue(float value, AxisBase axis) {
                String tradeDate = dataList.get((int) value % dataList.size()).getDataDT();
                return TextUtils.formatDateToMD(tradeDate,"yyyy-MM-dd HH:mm","MM-dd HH:mm");
            }
        });
        ...
        leftYAxis.setAxisMinimum(min - (max/8)); // start at zero
        leftYAxis.setAxisMaximum(max + (max/8)); // the axis maximum is 100
     ...
     * 添加曲线
     */
    public void addLineY(List<CompositeIndexBean> dataList, String name, int color,float min,float max) {
        List<Entry> entries = new ArrayList<>();
    ...
    public void addLine2(List<CompositeIndexBean> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            CompositeIndexBean data = dataList.get(i);
            Entry entry = new Entry(i, (float) data.getValue());
